...
- .NET 10 SDK
- AIKernel.NET contract packages `0.1.3`
...
Version 0.1.3 is the current canonical GPU integration line. AIKernel.Wasm uses
the NuGet patch line `0.1.3` while depending on AIKernel.NET,
AIKernel.Core, and AIKernel.Providers `0.1.3`. Use
`0.1.3-dev{buildNumber}` NuGet packages for local Wasm validation and
`0.1.3.dev{buildNumber}` `aikernel-wasm` wheels unless a separate PyPI patch
task is opened.
